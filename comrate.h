...
public:
  ComRate ();
  ComRate (const Point& center, double radius, double rate);
  ...
  virtual void set        (const Point& center, double radius, double rate);
  virtual void set        (const Circle& circle, double rate);
  virtual void set        (const ComRate& comrate);

  virtual double get_rate_at (const Point& point) const;
